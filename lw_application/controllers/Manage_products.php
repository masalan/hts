    ...
    */  
    ...
    function add($selectedCategory = '')
    {
        ...
        $val->set_rules(
            ...
            'trim|required|alpha_dash|is_unique[products.product_id]'
        );
        $val->set_rules(
            'product_price', __('Product Price'), 
            'trim|required|numeric|greater_than[0]'
        );  
        $val->set_rules('product_sizes', __('Product Sizes'), 'trim');
        $val->set_rules('product_color', __('Product Colors'), 'trim');
        $val->set_rules(
